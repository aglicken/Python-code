...
Assembler for DM2018S assembly language.
...
ASM_SYMBOLIC_PAT = re.compile(r"""
   ...
   # The instruction proper 
   \s*
    (?P<opcode>    (LOAD) | (STORE) | (JUMP))           # Opcode MADE CANGE
    (/ (?P<predicate> [a-zA-Z]+) )?   # Predicate (optional)
    \s+
    ((?P<target>    r[0-9]+),)?            # Target register
    # REMOVED 2 REGISTERS
...
PATTERNS = [(ASM_FULL_PAT, AsmSrcKind.FULL),
            ...
            (ASM_SYMBOLIC_PAT, AsmSrcKind.SYMBOLIC)  # ADDED!!
            ]
...
def build_table(lines) -> Tuple[dict, int]:
    ...
    for lnum in range(len(lines)):
        ...
        try:
            fields = parse_line(line)
            if fields["label"]:
                if fields["label"] in line_dict:
                    line_address += 1
                else:
                    line_dict[fields["label"]] = line_address
            ...
            error_ct += 1
            print("Unknonwn Label in line {}: {}".format(lnum,e))
        if error_ct > 4:
            print("Too many errors")
            sys.exit(1)

    return line_dict, error_ct

def transform_lines(lines: List[str], symple_table) -> None:
    ''' Transform lines in place with second pass over lines. Increase address counter where appropriate.
    '''
    address = 0
    for lnum in range(len(lines)):
        line = lines[lnum]
        ...
        if fields["kind"] == AsmSrcKind.SYMBOLIC:
            lines[lnum] = resolve_line(fields, address, symple_table)
    return None


def resolve_line(fields,address, symtable) -> str:
    ''' Takes dictionary of fields and returns a properly formatted string.
    '''
    label = fields["label"] or ""
    op = fields["opcode"]
    assert op == 'LOAD' or 'STORE' or 'JUMP', 'Operation is not allowed'
    symbol = fields["symbol"]
    target_reg = fields["target"]
    if fields["predicate"]:
        predicate = "/{}".format(fields["predicate"])
    else:
        predicate = ""
    if symbol in symtable:
        pc_mem = symtable[symbol] - address
    if op == "LOAD" or op == "STORE":
        return "{} {} {},r0,r15[{}]".format(label, op, target_reg, pc_mem)
    if op == "JUMP":
...
def main():
    ...
    symtable, errors = build_table(lines)
    if errors == 0:
        transform_lines(lines, symtable)
        for line in lines:
            print(line.rstrip(),file=args.objfile) # changed word to line and added .strip()

if __name__ == "__main__":
    main()


### py assembler_pass2.py programs/first.asm > testout ###