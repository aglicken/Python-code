...
class Tile(object):
    ...
    def set_value(self, value, guess=False):
        if value in CHOICES:
            self.value = value
            self.candidates = set([value]) # set of str objects
        else:
            self.value = UNKNOWN
    ...
    def eliminate(self, choices: Set[str]) -> bool:
        ...
        start_candi = self.candidates.copy()

        self.candidates = self.candidates - choices # update tile candidates

        if start_candi == self.candidates:
            return False
